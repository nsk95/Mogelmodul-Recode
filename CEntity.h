...
public:
	...
	uint32_t getEntityByIndex(unsigned __int8 index = 1);

	int getHealth(std::uintptr_t entity);

	int getTeam(std::uintptr_t entity);

	Vector3D getBonePos(int entity, int boneid);

	int getClassID(std::uintptr_t entity) const;

	int getHealth2(const uint32_t * entity);

	int getTeam2(const uint32_t * entity);

	void setGlow(int index, float red, float blue, float green, float alpha, bool a1, bool a2);

};

extern CEntity* g_pEntity;